...
pathFT <- paste("/code/Projects/3i/Panel_T-cell_MLN/Results/FlowType")
...
Genotypes <- c(WildTypeTypes, KnockOutTypes) # Contains the names of Wild Types and Knock-outs
#GenotypesLong <- c(allWildTypes, allKnockOuts) # Contains the whole path of all the FT files

allFT <- c(allWildTypes[,1], allKnockOuts[,1])  # Contains the whole path of all the FT files with 418 WTs at the beginning
...
# Storing the Proportions for each phenotype and for each FCS file in the Matrix3iTcell
