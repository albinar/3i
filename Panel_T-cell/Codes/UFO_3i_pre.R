...
WT_dates2 <- strsplit(as.character(WT_dates), split="-") ## The year format in some rows was "2014" instead of just "14"; hence, had to add the WT_dates2.temp
WT_dates2.temp <- sapply(1:length(WT_dates2), function(x){as.numeric(WT_dates2[[x]][3])})
WT_dates2.temp[which(WT_dates2.temp > 2000)] <- WT_dates2.temp[which(WT_dates2.temp > 2000)]-2000

WT_dates3 <- NULL
months <- c(31,28,31,30,31,30,31,31,30,31,30,31)
for ( q1 in 1:length(WT_dates2) ) {
        temp <- WT_dates2[[q1]]
        WT_dates3[q1] <- (as.numeric(WT_dates2.temp[q1])-14)*365 + sum(months[ 1:match(temp[2], month.abb)] ) - months[match(temp[2], month.abb)] + as.numeric(temp[1])
}
...
tempfunc <- function(CSN){
    ...
    subsample = sample(seq(nrow(exprs(f))), 10^3) ## changsubsample from 10^3 to 10^4
    x <- exprs(f)[subsample,]
...
library(snow)
library(Rmpi)
cl <- makeCluster(spec=5, type = "SOCK")
