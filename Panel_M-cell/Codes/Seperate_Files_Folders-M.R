...
FCS.Groups <- "FCS_Groups_SPLEEN"
dir.create (FCS.Groups)

# path to the files sent on August 27, 2015
pathMice <- paste("/mnt/f/FCS data/IMPC/IMPC_2016_Pilot_Data/KCL_WTSI_2015-08-27/Myeloid SPLN Labelled")
...
pathMice <- paste("/mnt/f/FCS data/IMPC/IMPC_2016_Pilot_Data/KCL_WTSI_2016-03-08/SPLN M 169+")
# Reads all folders and files in current path folder and makes a list of all of their paths
allMice <- dir(pathMice, full.names=T, recursive=T, pattern = "*.fcs")
...
pathMice <- paste("/mnt/f/FCS data/IMPC/IMPC_2016_Pilot_Data/KCL_WTSI_2016-10-11/SPLM M Labelled")
# Reads all folders and files in current path folder and makes a list of all of their paths
allMice <- dir(pathMice, full.names=T, recursive=T, pattern = "*.fcs")
...
CSVfile3 <- read.csv("/mnt/f/FCS data/IMPC/IMPC_2016_Pilot_Data/KCL_WTSI_2016-10-11/attachments/download_phnSpleenImmuno.csv")
CSVfile3 <- as.matrix(CSVfile3)
Genotype3 <- CSVfile3[,c(11)]
Genotype3 <- sub("/","_", Genotype3)
Mouse_Label3 <- CSVfile3[,c(20)]
...
suppressWarnings(dir.create ( "Results_SPLEEN/"))
save ( store.allMice , file =  paste("Results_SPLEEN/store.allMice.Rdata",sep="") )
save(Genotype, file = paste("Results_SPLEEN/Genotype.Rdata", sep = ""))
save(uniqueGT, file = paste("Results_SPLEEN/uniqueGT.Rdata", sep = ""))
save(Mouse_Label, file = paste("Results_SPLEEN/Mouse_Label.Rdata", sep = ""))
## Saving the two combined spreadsheets
save(CSVfile, file = paste("Results_SPLEEN/CSVfile.Rdata", sep = ""))
