...
#loop.idx <- c(92, 148, 486, 570, 582, 996,1378,1661,1727, 2002, 1277, 914, 49,167,932,1054,1449,1451,1457,1458,1459)
#loop.idx <- c(58,99,100,148,570,702,709,1250,1251,1577,1578,996)
#loop.idx <-c(1,80,115,126,348,352,415,996,1001,1900,10,453,24,69,42,173,692,875,1585,991, 1455)
#loop.idx <- 1:nrow(store.allFCS)
#loop.idx <- c(791,  139,  208,  344,  383,  498,  751,  882,  890, 1044, 1073, 1379, 1406, 1452, 1532, 1724, 1955, 1957) 
loop.idx <- c(946)
...
for(i in loop.idx){
  ...
  possibleError <- tryCatch({
    ...
    load(file = paste("Results_MLN/After_Clean/", store.allFCS[i,1], "/AftClean_", store.allFCS[i,2], ".Rdata", sep=""))
    ...
    fsc.a.gate.low <- max(deGate(live, channel = c(1), all.cut = T, upper = F))
    ...
    if(fsc.a.gate.low > 60000){
      temp <- deGate(live, channel = c(1), all.cut = T, upper = F)
      if(length(temp) > 1){
    ...
    # CD45
    cd45.gate.low <- max(deGate(lymph, channel = c(14), all.cut = T, upper=F))
    ...
    if(!is.null(names(linneg.macneg.gate.slanta))){ # using 95% cutoff
      linneg.macneg.gate.slanta <- deGate(linneg.macneg.s, channel=c(4), upper = F) - 0.2
    }
    
    eosinTempb0 <- flowDensity(linneg.macneg.s, channels =c(9, 4), position = c(NA, T), gates = c(NA, linneg.macneg.gate.slanta))
    linneg.macneg.gate.slantb <- deGate(eosinTempb0@flow.frame, channel=c(9))
    ...
    unknown <- flowDensity(linneg.macneg, channels = c(9,4), position = c(T, F), gates = c(1.8,50000))
    
    # Trace back where unknown comes from
    plotDens(linneg.macneg, channels = c(9,4), main = 'linneg.macneg')
    points(linneg.Tempb@flow.frame@exprs[unknown@index, c(9,4)], col=2, cex=0.2)
    
    plotDens(cd45, channels = c(14,12), main="CD45", cex.lab = 2, cex.axis = 2, cex.main=2)
    lines(cd45sTempb@filter, lwd=2)
    points(cd45sTempb@flow.frame@exprs[unknown@index, c(14,12)], col=2, cex=0.2)
    
    plotDens(lymph, channels = c(14,12), main='Lymphocytes', cex.lab = 2, cex.axis = 2, cex.main=2)
    abline(v=cd45.gate.low, lwd=2); abline(v=cd45.gate.high, lwd=2)
    abline(h= lincd3cd109cd161.gate.high, lwd=2)
    points(cd45sTempb@flow.frame@exprs[unknown@index, c(14,12)], col=2, cex=0.2)
    
    plotDens(live, c(1,4), main = "Live", cex.lab = 2, cex.axis = 2, cex.main=2)
    abline(v=fsc.a.gate.low, lwd=2); abline(v=fsc.a.gate.high, lwd=2)
    abline(h=ssc.a.gate, lwd=2)
    points(live.flowD@flow.frame@exprs[unknown@index, c(1,4)], col=2, cex=0.2)
    
    plotDens(singlets,  c(11,4), main= "Singlets", cex.lab = 2, cex.axis = 2, cex.main=2)
    abline(v=live.gate, lwd=2)
    points(singlets.flowD.l@flow.frame@exprs[unknown@index, c(11,4)], col=2, cex=0.2)
    
    plotDens(f,c("SSC-A","SSC-W"), main="Ungated", cex.lab = 2, cex.axis = 2, cex.main=2)
    abline(h = singlets.gate.l, lwd=2)
    abline(h = singlets.gate.h, lwd=2)
    points(f@exprs[unknown@index, c("SSC-A","SSC-W")], col=2, cex=0.2)
    ...
    Ly6c.gate <- deGate(temp@flow.frame, channel = c(10), all.cut = T)
    ...
    if(Ly6c.gate < 2.1){
      ...
      temp.gate <- deGate(temp@flow.frame, channel = c(10), all.cut = T)
    ...
    cd317.gate <- deGate(temp@flow.frame, channel = c(15))
    if(cd317.gate < 1.7){ # gate is lower than expected
      if(!is.null(names(cd317.gate))){ # using 95% cutoff - suggests a large cd317 lo population
        maxDens <- density((temp@flow.frame)@exprs[, 15]) 
        if(max(maxDens$x)>2){
    ...
    temp <- deGate(cDC.flowD.temp@flow.frame, channel = c(16), all.cut = T)
    ...
    cd11b.gate.low <- deGate(cd8a.typedc, channel = c(13), upper = F) + 0.1
  ...
  tryCatch({
    #--------Start Big Png------------
    png ( file = paste("Results/Figures/ScatterPlots/", store.allFCS[i,1], "/", "Total_", store.allFCS[i,2], ".png", sep = "" ), width=2100, height=2100*3/6)
    ...
    lines(linneg.Tempb@filter,lwd=2)
    
    # Plotting Ly6G_SSC-A
    #     plotDens(linneg.macneg, channels = c(9,4), main="Lin-Mac-")
    #     lines(eosinTempb@filter, lwd=2)
    #     segments(Ly6G.gate,par('usr')[3],Ly6G.gate,ssc.a.gate2, lwd=2); segments(par('usr')[1],ssc.a.gate2,Ly6G.gate,ssc.a.gate2, lwd=2)
    #     neu2.gate <- Ly6G.gate
    #     segments(neu2.gate,par('usr')[3],neu2.gate,ssc.a.gate3, lwd=2); segments(neu2.gate,ssc.a.gate3,par('usr')[2],ssc.a.gate3, lwd=2)
    ...
    #plotDens(linneg.macneg, channels = c(9,4), main="Lin-Mac-")
    #lines(Ly6Gneg.flowD@filter,lwd=2)
    #lines(eosinTempb@filter, lwd=2)
    #lines(neu2.flowD@filter, lwd=2)
    
    # Plotting Ly6C_CD11b
    plotDens(Ly6Gneg, channels = c(10,13), main="Ly6G-", cex.lab = 2, cex.axis = 2, cex.main=2)
    #lines(mono.Ly6c.hi.flowD@filter,lwd=2)
    segments(Ly6c.gate, cd11b.gate, Ly6c.gate, par('usr')[4], lwd=2)
    segments(Ly6c.gate, cd11b.gate, par('usr')[2], cd11b.gate, lwd=2)
    ...
    segments(Ly6c.gate2, cd317.gate, Ly6c.gate2, par('usr')[4], lwd=2)
    segments(Ly6c.gate2, cd317.gate, par('usr')[2], cd317.gate, lwd=2)
    
    # Plotting Ly6C_CD317
    plotDens(not.pDC, channels = c(16, 7), main="NOT pDC", cex.lab = 2, cex.axis = 2, cex.main=2)
    #lines(cDC.flowD@filter,lwd=2)
    segments(cd11c.gate, mhcII.gate, cd11c.gate, par('usr')[4], lwd=2)
    segments(cd11c.gate, mhcII.gate, par('usr')[2], mhcII.gate, lwd=2)
    
    # Plotting CD11b_CD86
    plotDens(cDC, channels = c(13, 18), main="cDC", cex.lab = 2, cex.axis = 2, cex.main=2)
    lines(cd8a.typedc.Tempb@filter, lwd = 2)
