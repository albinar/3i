...
#dir.create ( "Results/Figures/FinalTableOfProportions")
...
PlotNames <- c("Percentage of Events in Each Sub-Population Out of the Total", "", "Percentage of Events in Each Sub-Population Out of CD45", "", "Percentage of Events in Each Sub-Population Out of the Parent Population", "", "Cell Count of each sub-population", "")

PlotYaxis   <- c("Log10 (Percentage)","","Log10 (Percentage)","","Log10 (Percentage)","","Log10 (Cell Count)")
PlotYaxisNP <- c("Percentage","","Percentage","","Percentage","","Cell Count")
l <- nrow(FinalTable)
FinalTableOfProportions <- FinalTable

for ( w2 in c(1,3,5,7) ) {
    dudeTitle <- colnames(FinalTableOfProportions)[2:ncol(FinalTableOfProportions)]#as.matrix(FinalTableOfProportions[w2,])[2:34] 
    dudeTitle <- rbind(paste(dudeTitle, "MA"),paste(dudeTitle,"AA"))
    dudeTitle <- as.vector(dudeTitle)
    dude1 <- as.numeric(as.matrix(FinalTableOfProportions[seq(w2+8,w2+(l-8), by = 8),3]))
#     dude1[which(is.na(dude1))] <- 0
    dude <- data.frame(log10(dude1))
    NormalPercentagesData <- data.frame(dude1)

        
    dude1b <- as.numeric(as.matrix(FinalTableOfProportions[seq(w2+1+8,w2+1+(l-8), by = 8),3]))
#     dude1b[which(is.na(dude1b))] <- 0
    ...
    for ( w1 in 3:ncol(FinalTableOfProportions)){

        dude2 <- as.numeric(as.matrix(FinalTableOfProportions[seq(w2+8,w2+(l-8), by = 8),w1]))
#         dude2[which(is.na(dude2))] <- 0
        dude <- data.frame(dude, log10(dude2))
        NormalPercentagesData <- data.frame(NormalPercentagesData, dude2)
        
        dude3 <- as.numeric(as.matrix(FinalTableOfProportions[seq(w2+1+8,w2+1+(l-8), by = 8),w1]))
#         dude3[which(is.na(dude3))] <- 0
        dude <- data.frame(dude, log10(dude3))
        NormalPercentagesData <- data.frame(NormalPercentagesData, dude3)
    }
    # Log Percentages
        png ( file = paste("Results/Figures/FinalTableOfProportions/Box_Plot_", gsub(" ", "_", PlotNames[w2]),".png", sep = "" ), height=1000, width=1600 )
        par(mar=c(35,5,2,2)) # margins
        if (w2 <= 6) {
            boxplot(dude[,-c(1,2)], cex=0.2, pch=19,  col=rep(c("Blue","red"),(ncol(FinalTableOfProportions)-3)), main=paste("Box Plots: ", PlotNames[w2], sep=""), ylab=PlotYaxis[w2], axes=FALSE, frame.plot=FALSE,las=2,na.action=na.omit)
        } else {
            boxplot(dude[,], cex=0.2, pch=19,  col=rep(c("Blue","red"),(ncol(FinalTableOfProportions)-3)), main=paste("Box Plots: ", PlotNames[w2], sep=""), ylab=PlotYaxis[w2], axes=FALSE, frame.plot=FALSE,las=2,na.action=na.omit)
        }
    ...
    #     axis(1,at=1:33)
        if(w2 <= 4) {
            axis(1,at=1:26,labels=dudeTitle[-c(1,2)],las=2)
        } else {
            axis(1,at=1:28,labels=dudeTitle,las=2)
        }
        axis(2,at=(-17:17)*1)
        dev.off()
    # Regular Percentages
        png ( file = paste("Results/Figures/FinalTableOfProportions/Box_Plot_", gsub(" ", "_", PlotNames[w2]),"_Normal_Percentage.png", sep = "" ), height=1000, width=1600 )
        par(mar=c(35,5,2,2)) # margins
        if (w2 <= 6) {
    ...
    dudeMice <- as.matrix(FinalTableOfProportions[seq(7,1+(l-6), by = 6),1])
