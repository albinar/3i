...
source("/data/Panel_M-cell/Codes/preProcessingFunc.R")
## Function for creating a Global Frame
source("/data/Panel_M-cell/Codes/globalFrameFunc.R")
## Function for TimeOutput in order to determine the execution time 
source("/data/IMPC_Sanger/3iTcellfunctions.R")
...
outputPath <- "/mnt/f/FCS data/IMPC/IMPC-Results/3i/Panel_M-cell/MLN/"

## Paths to the FCS files sent on Spleen Organ of August 2015, MArch 2016, and October 2016
inPath1 <- "/mnt/f/FCS data/IMPC/IMPC_2016_Pilot_Data/KCL_WTSI_Sanger/KCL_WTSI_2015-08-27/Myeloid MLN Labelled"
inPath2 <- "/mnt/f/FCS data/IMPC/IMPC_2016_Pilot_Data/KCL_WTSI_Sanger/KCL_WTSI_2016-03-08/MLN M 169+"
inPath3 <- "/mnt/f/FCS data/IMPC/IMPC_2016_Pilot_Data/KCL_WTSI_Sanger/KCL_WTSI_2016-10-11/MLN M Labelled"
inPath4 <- "/mnt/f/FCS data/IMPC/IMPC_2016_Pilot_Data/KCL_WTSI_Sanger/KCL_WTSI_2017-07-19/MLN 287 M"
...
# # Reading a single spreadsheet containing the metadata information for the complete 3i MLN data set
CSVfile <- read.csv("/mnt/f/FCS data/IMPC/IMPC_2016_Pilot_Data/KCL_WTSI_Sanger/attachments/20170725_phnMLNImmuno.csv")
CSVfile <- as.matrix(CSVfile)

preProcessing.Output <- preProcessingFunc(inputPath, CSVfile, no_cores)
...
n.FlowReposFiles <- nrow(store.allFCS) + nrow(notListed.FCS) + NObarcodes.FCS + nrow(duplicate.FCS) + nrow(lessCells.FCS) #+ nrow(corrupted.FCS)
print(paste0("Total number of files received through flowRepository: ", n.FlowReposFiles))
n.WTfiles <-  length(which(store.allFCS[ ,'Genotype'] == "WT"))
print(paste0("There are in total ", nrow(store.allFCS), " files for analysis: ", n.WTfiles, 
             " are WTs and ", (nrow(store.allFCS) -n.WTfiles), " are KOs"))
...
print(paste0("Total Number of unique KOs: ", length(uniqueGT) - 2))  
...
save(NObarcodes.FCS, file = paste0(outputPath, "Results/NObarcodes.FCS.Rdata"))
