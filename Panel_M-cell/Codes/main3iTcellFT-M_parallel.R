...
suppressWarnings(dir.create("Results_SPLEEN/Figures/ScatterPlots_Updated"))

load("Results_SPLEEN/uniqueGT.Rdata")
load("Results_SPLEEN/store.allFCS.Rdata")
load(file =  paste("Results_SPLEEN/ThreshTableUpdated.Rdata",sep="") ) 
rownames(gthres) <- store.allFCS[,2]
gthres <- gthres[,-c(1,2)]
...
invisible(sapply(1:length(uniqueGT), function(x){
  ...
  suppressWarnings(dir.create ( paste("Results_SPLEEN/Figures/ScatterPlots_Updated/", uniqueGT[x], sep="") ))}))
...
props.events.MFIs <- ddply(file.names, "FCS.file", function(x){

  all.props.updated <- matrix(data = NA, nrow = 1, ncol = 20)
  all.events.updated <- matrix(data = NA, nrow = 1, ncol = 20)
  MFI.mean.OriginalData <- matrix(data = NA, nrow = 342, ncol = 1)
  MFI.median.OriginalData <- matrix(data = NA, nrow = 342, ncol = 1)
  MFI.mean.TransformedData <- matrix(data = NA, nrow = 342, ncol = 1)
  MFI.median.TransformedData <- matrix(data = NA, nrow = 342, ncol = 1)
  ...
  load(file =  paste("Results_SPLEEN/Before_Transform/", x$Genotype,"/BfrTrans_", x$FCS.file,".Rdata",sep="") )
  
  possibleError <- tryCatch({
    
    all.events.updated[1] <- nrow(f)
    all.props.updated[1] <- 1
    
    ## 1st method of plotting FSC-A_SSC-W
    singlets.flowD.h <- flowDensity(f, channels = c("SSC-A", "SSC-W"), position = c(NA, F), gates = c(NA, gthres[x$FCS.file,1]))
    singlets.flowD.l <- flowDensity(singlets.flowD.h, channels = c("SSC-A", "SSC-W"), position = c(NA, T), gates = c(NA, gthres[x$FCS.file,2]))
    singlets.flowD.l@proportion <- (singlets.flowD.l@cell.count/nrow(f))*100
    singlets <- singlets.flowD.l@flow.frame
    
    all.props.updated[2] <- singlets.flowD.l@proportion
    all.events.updated[2] <- singlets.flowD.l@cell.count
    
    ###############################################################################################
    ## Gating Singlets. Plotting Live/Dead_SSC-A
    live.flowD <- flowDensity(singlets, channels = c(11, 4), position = c(F, NA), gates = c(gthres[x$FCS.file,3], NA))
    live <- live.flowD@flow.frame
    
    all.props.updated[3] <- live.flowD@proportion
    all.events.updated[3] <- live.flowD@cell.count
    
    ###############################################################################################
    ## Gating Live. Plotting FSC-A_SSC-A
    temp <- flowDensity(live, channels = c(1, 4), position = c(T, F), gates = c(gthres[x$FCS.file,5], gthres[x$FCS.file,6]))
    lymph.flowD <- flowDensity(temp, channels = c(1, 4), position = c(F, F), gates = c(gthres[x$FCS.file,4], gthres[x$FCS.file,6]))
    lymph.flowD@proportion <- (lymph.flowD@cell.count/live.flowD@cell.count)*100
    lymph <- lymph.flowD@flow.frame
    
    all.props.updated[4] <- lymph.flowD@proportion
    all.events.updated[4] <- lymph.flowD@cell.count
    
    ###############################################################################################
    ## Gating Lymphocytes. Plotting CD45_Lin(CD3, CD109, CD161)
    temp <- flowDensity(lymph, channels = c(14, 12), position = c(T, F), gates = c(gthres[x$FCS.file,7], gthres[x$FCS.file,9]))
    cd45.flowD <- flowDensity(temp, channels = c(14, 12), position = c(F, F), gates = c(gthres[x$FCS.file,8], gthres[x$FCS.file,9]))
    cd45 <- cd45.flowD@flow.frame
    cd45.flowD@proportion <- (cd45.flowD@cell.count/lymph.flowD@cell.count)*100
    
    all.props.updated[5] <- cd45.flowD@proportion
    all.events.updated[5] <- cd45.flowD@cell.count
    ...
    cd45sTempb <- flowDensity(cd45s, channels=c(14, 12),position=c(NA, F), gates = c(NA, gthres[x$FCS.file,10]))
    ...
    all.props.updated[6] <- cd45sTempb@proportion
    all.events.updated[6] <- cd45sTempb@cell.count
    ...
    linneg.Tempb <- flowDensity(lin.neg.s, channels=c(8, 13),position=c(F, NA), gates = c(gthres[x$FCS.file,11], NA))
    ...
    all.props.updated[7] <- linneg.Tempb@proportion
    all.events.updated[7] <- linneg.Tempb@cell.count
    ...
    all.props.updated[8] <- RP.mac.cell.count/cd45sTempb@cell.count*100
    all.events.updated[8] <- RP.mac.cell.count
...
#     points(cd45sTempb@flow.frame@exprs[RP.mac.ind, c(8,13)], col=3, cex=0.2)
    ...
    eosinTempb0 <- flowDensity(linneg.macneg.s, channels =c(9, 4), position = c(NA, T), gates = c(NA, gthres[x$FCS.file,12]))
    eosinTempb <-flowDensity(eosinTempb0, channels =c(9, 4), position = c(T, NA), gates = c(gthres[x$FCS.file,13], NA)) 
    ...
    Ly6GnegTempb <-flowDensity(eosinTempb0, channels =c(9, 4), position = c(F, NA), gates = c(gthres[x$FCS.file,13], NA)) 
    ...
    if(ncells.neu2 > 1){ 
      neu2Tempb <- flowDensity(linneg.macneg.s, channels =c(9, 4), position = c(NA, F), gates = c(NA, gthres[x$FCS.file,12]))
      ...
      all.events.updated[10] <- neu2Tempb@cell.count
      all.props.updated[10] <- neu2Tempb@cell.count/linneg.Tempb@cell.count*100
    }else{
      all.events.updated[10] <- ncells.neu2
      all.props.updated[10] <- ncells.neu2/linneg.Tempb@cell.count*100
    }

    all.props.updated[9] <- Ly6GnegTempb@cell.count/linneg.Tempb@cell.count*100
    all.props.updated[11] <- eosinTempb@cell.count/linneg.Tempb@cell.count*100
    all.events.updated[9] <- Ly6GnegTempb@cell.count 
    all.events.updated[11] <- eosinTempb@cell.count
...
#     points(cd45sTempb@flow.frame@exprs[Ly6GnegTempb@index, c(9,4)], col=4, cex=0.2)
    ...
    mono.Ly6c.hi.flowD <- flowDensity(Ly6Gneg, channels = c(10, 13), position = c(T, T), gates = c(gthres[x$FCS.file,14], gthres[x$FCS.file,15]))
    mono.Ly6c.hi <- mono.Ly6c.hi.flowD@flow.frame
    
    all.props.updated[12] <- mono.Ly6c.hi.flowD@proportion
    all.events.updated[12] <- mono.Ly6c.hi.flowD@cell.count
    ...
    all.props.updated[13] <- not.mono.Ly6c.hi.flowD@cell.count/Ly6GnegTempb@cell.count*100
    all.events.updated[13] <- not.mono.Ly6c.hi.flowD@cell.count
    
    ############################################################################################
    ## Gating NOT(Monocytes Ly6c hi). Plotting Ly6C_CD3
    
    pDC.flowD <- flowDensity(not.mono.Ly6c.hi, channels = c(10, 15), position = c(T, T), gates = c(gthres[x$FCS.file,16], gthres[x$FCS.file,17]))
    pDC <- pDC.flowD@flow.frame
    
    all.props.updated[14] <- pDC.flowD@proportion
    all.events.updated[14] <- pDC.flowD@cell.count
    ...
    all.props.updated[15] <-  not.pDC.flowD@cell.count/not.mono.Ly6c.hi.flowD@cell.count*100
    all.events.updated[15] <- not.pDC.flowD@cell.count
...
#     points(not.mono.Ly6c.hi.flowD@flow.frame@exprs[not.pDC.flowD@index, c(10,15)], col=2, cex=0.2)
    ...
    cDC.flowD <- flowDensity(not.pDC, channels = c(16, 7), position = c(T, T), gates = c(gthres[x$FCS.file,19],  gthres[x$FCS.file,18]))
    cDC <- cDC.flowD@flow.frame
    
    all.props.updated[16] <- cDC.flowD@proportion
    all.events.updated[16] <- cDC.flowD@cell.count
...
#     misc@exprs <- misc@exprs[-cDC.flowD@index, ]
    ...
    all.props.updated[17] <- misc.flowD@cell.count/not.pDC.flowD@cell.count*100
    all.events.updated[17] <- misc.flowD@cell.count
    ...
    R <- matrix(c(cos(theta0), sin(theta0), -sin(theta0), cos(theta0)), 2 , 2)
      ...
      cd8a.typedc.Tempb <- flowDensity(cDC.s, channels=c(13, 18),position=c(NA, T), gates = c(NA, gthres[x$FCS.file,20]))
    ...
    all.props.updated[18] <- cd8a.typedc.Tempb@proportion
    all.events.updated[18] <- cd8a.typedc.Tempb@cell.count
...
#     cd11b.plus.cd86.lo@exprs <- cd11b.plus.cd86.lo@exprs[-cd8a.typedc.Tempb@index, ]
    ...
    all.props.updated[19] <- cd11b.plus.cd86.lo.flowD@cell.count/cDC.flowD@cell.count*100
    all.events.updated[19] <- cd11b.plus.cd86.lo.flowD@cell.count
    
    ############################################################################################
    ## Gating CD8A Type DC. Plotting CD103_CD11b to obtain CD103+ DC
    
    cd103plus.dc.flowD <- flowDensity(cd8a.typedc, channels = c(17, 13), position = c(T, NA), gates = c(gthres[x$FCS.file,21], NA))
    cd103plus.dc <- cd103plus.dc.flowD@flow.frame
    
    all.props.updated[20] <- cd103plus.dc.flowD@proportion
    all.events.updated[20] <- cd103plus.dc.flowD@cell.count
    ...
    MFI.mean.OriginalData <- c(singlets.MFI, live.MFI, lymph.MFI, cd45.MFI, linneg.MFI, linneg.macneg.MFI, RPmac.MFI, Ly6Gneg.MFI, neu2.MFI,
    ...
    MFI.median.OriginalData <- c(singlets.MFI, live.MFI, lymph.MFI, cd45.MFI, linneg.MFI, linneg.macneg.MFI, RPmac.MFI, Ly6Gneg.MFI, neu2.MFI,
                                    eosin3.MFI, mono.Ly6c.hi.MFI, not.mono.Ly6c.hi.MFI, pDC.MFI, not.pDC.MFI, cDC.MFI, misc.MFI, CD8A.TypeDC.MFI, 
                                    CD11bplus.CD86lo.MFI, CD103plusDC.MFI)   
    
    TC.chan <- 2 # Number of channels which Time/Cleaning take up  
  
    # Saving the MFIs (mean) of Transformed (Data after Transformation)
    singlets.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){mean(f@exprs[singlets.flowD.l@index, x], na.rm = T)})
    live.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){mean(f@exprs[live.flowD@index, x], na.rm = T)})
    lymph.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){mean(f@exprs[lymph.flowD@index, x], na.rm = T)})
    cd45.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){mean(f@exprs[cd45.flowD@index, x], na.rm = T)})
    linneg.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){mean(f@exprs[cd45sTempb@index, x], na.rm = T)})
    linneg.macneg.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){mean(f@exprs[linneg.Tempb@index, x], na.rm = T)})
    RPmac.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){mean(f@exprs[RP.mac.ind, x], na.rm = T)})
    Ly6Gneg.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){mean(f@exprs[Ly6GnegTempb@index, x], na.rm = T)})
    neu2.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){mean(f@exprs[neu2Tempb@index, x], na.rm = T)})
    eosin3.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){mean(f@exprs[eosinTempb@index, x], na.rm = T)})
    mono.Ly6c.hi.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){mean(f@exprs[mono.Ly6c.hi.flowD@index, x], na.rm = T)})
    not.mono.Ly6c.hi.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){mean(f@exprs[not.mono.Ly6c.hi.flowD@index, x], na.rm = T)})
    pDC.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){mean(f@exprs[pDC.flowD@index, x], na.rm = T)})
    not.pDC.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){mean(f@exprs[not.pDC.flowD@index, x], na.rm = T)})
    cDC.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){mean(f@exprs[cDC.flowD@index, x], na.rm = T)})
    misc.MFI <-  sapply(1:(ncol(f@exprs)- TC.chan), function(x){mean(f@exprs[misc.flowD@index, x], na.rm = T)})
    CD8A.TypeDC.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){mean(f@exprs[cd8a.typedc.Tempb@index, x], na.rm = T)})
    CD11bplus.CD86lo.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){mean(f@exprs[cd11b.plus.cd86.lo.flowD@index, x], na.rm = T)}) 
    CD103plusDC.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){mean(f@exprs[cd103plus.dc.flowD@index, x], na.rm = T)}) 
    
    MFI.mean.TransformedData <- c(singlets.MFI, live.MFI, lymph.MFI, cd45.MFI, linneg.MFI, linneg.macneg.MFI, RPmac.MFI, Ly6Gneg.MFI, neu2.MFI,
                                    eosin3.MFI, mono.Ly6c.hi.MFI, not.mono.Ly6c.hi.MFI, pDC.MFI, not.pDC.MFI, cDC.MFI, misc.MFI, CD8A.TypeDC.MFI, 
                                    CD11bplus.CD86lo.MFI, CD103plusDC.MFI)
    
    # Saving the MFIs (median) of Transformed (Data after Transformation)
    singlets.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){median(f@exprs[singlets.flowD.l@index, x], na.rm = T)})
    live.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){median(f@exprs[live.flowD@index, x], na.rm = T)})
    lymph.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){median(f@exprs[lymph.flowD@index, x], na.rm = T)})
    cd45.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){median(f@exprs[cd45.flowD@index, x], na.rm = T)})
    linneg.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){median(f@exprs[cd45sTempb@index, x], na.rm = T)})
    linneg.macneg.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){median(f@exprs[linneg.Tempb@index, x], na.rm = T)})
    RPmac.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){median(f@exprs[RP.mac.ind, x], na.rm = T)})
    Ly6Gneg.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){median(f@exprs[Ly6GnegTempb@index, x], na.rm = T)})
    neu2.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){median(f@exprs[neu2Tempb@index, x], na.rm = T)})
    eosin3.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){median(f@exprs[eosinTempb@index, x], na.rm = T)})
    mono.Ly6c.hi.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){median(f@exprs[mono.Ly6c.hi.flowD@index, x], na.rm = T)})
    not.mono.Ly6c.hi.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){median(f@exprs[not.mono.Ly6c.hi.flowD@index, x], na.rm = T)})
    pDC.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){median(f@exprs[pDC.flowD@index, x], na.rm = T)})
    not.pDC.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){median(f@exprs[not.pDC.flowD@index, x], na.rm = T)})
    cDC.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){median(f@exprs[cDC.flowD@index, x], na.rm = T)})
    misc.MFI <-  sapply(1:(ncol(f@exprs)- TC.chan), function(x){median(f@exprs[misc.flowD@index, x], na.rm = T)})
    CD8A.TypeDC.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){median(f@exprs[cd8a.typedc.Tempb@index, x], na.rm = T)})
    CD11bplus.CD86lo.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){median(f@exprs[cd11b.plus.cd86.lo.flowD@index, x], na.rm = T)}) 
    CD103plusDC.MFI <- sapply(1:(ncol(f@exprs)- TC.chan), function(x){median(f@exprs[cd103plus.dc.flowD@index, x], na.rm = T)}) 
    
    MFI.median.TransformedData <- c(singlets.MFI, live.MFI, lymph.MFI, cd45.MFI, linneg.MFI, linneg.macneg.MFI, RPmac.MFI, Ly6Gneg.MFI, neu2.MFI,
                                      eosin3.MFI, mono.Ly6c.hi.MFI, not.mono.Ly6c.hi.MFI, pDC.MFI, not.pDC.MFI, cDC.MFI, misc.MFI, CD8A.TypeDC.MFI, 
                                      CD11bplus.CD86lo.MFI, CD103plusDC.MFI)   
    ...
    #--------Start Big Png------------
    png ( file = paste("Results_SPLEEN/Figures/ScatterPlots_Updated/",  x$Genotype, "/", "Total_", x$FCS.file, ".png", sep = "" ), width=2500, height=2500*3/5)
    par(mfrow=c(3,5),mar=(c(5, 5, 4, 2) + 0.1))
    
    plotDens(f,c("SSC-A","SSC-W"), main="Ungated", cex.lab = 2, cex.axis = 2, cex.main=2)
    abline(h = gthres[x$FCS.file,2], lwd=1)
    abline(h = gthres[x$FCS.file,1], lwd=1)
    
    # Plotting Live/Dead_SSC-A 
    plotDens(singlets,  c(11,4), main= "Singlets", cex.lab = 2, cex.axis = 2, cex.main=2)
    abline(v=gthres[x$FCS.file,3], lwd=1); 
    
    # Plotting FSC-A_SSC-A
    plotDens(live, c(1,4), main = "Live", cex.lab = 2, cex.axis = 2, cex.main=2)
    abline(v=gthres[x$FCS.file,5], lwd=1); abline(v=gthres[x$FCS.file,4], lwd=1)
    abline(h=gthres[x$FCS.file,6], lwd=1)
    
    # Plotting CD45_Lin(CD3,CD19,CD161)
    plotDens(lymph, channels = c(14,12), main='Lymphocytes', cex.lab = 2, cex.axis = 2, cex.main=2)
    abline(v=gthres[x$FCS.file,7], lwd=1); abline(v=gthres[x$FCS.file,8], lwd=1)
    abline(h=gthres[x$FCS.file,9], lwd=1)
    ...
    segments(gthres[x$FCS.file,14], gthres[x$FCS.file,15], gthres[x$FCS.file,14], par('usr')[4], lwd=1)
    segments(gthres[x$FCS.file,14], gthres[x$FCS.file,15], par('usr')[2], gthres[x$FCS.file,15], lwd=1)
    ...
    segments(gthres[x$FCS.file,16], gthres[x$FCS.file,17], gthres[x$FCS.file,16], par('usr')[4], lwd=1)
    segments(gthres[x$FCS.file,16], gthres[x$FCS.file,17], par('usr')[2], gthres[x$FCS.file,17], lwd=1)
    
    # Plotting CD11b_MHCII
    plotDens(not.pDC, channels = c(16, 7), main="NOT pDC", cex.lab = 2, cex.axis = 2, cex.main=2)
    segments(gthres[x$FCS.file,19], gthres[x$FCS.file,18], gthres[x$FCS.file,19], par('usr')[4], lwd=1)
    segments(gthres[x$FCS.file,19], gthres[x$FCS.file,18], par('usr')[2], gthres[x$FCS.file,18], lwd=1)
...
#     abline(v=gthres[x$FCS.file,21], lwd=1)
#     
  ...
  if (Do_flowType == T) {
    ...
    listGthres <- list(gthres[x$FCS.file,'linneg.macneg.gate.slanta'], gthres[x$FCS.file,'linneg.macneg.gate.slantb'], cd11b.gate.s,  
                    min(gthres[x$FCS.file,'Ly6c.gate'],gthres[x$FCS.file,'Ly6c.gate2']), cd317.gate2, gthres[x$FCS.file,'mhcII.gate'],
                    gthres[x$FCS.file,'cd11c.gate'], gthres[x$FCS.file,'cDC.gate.slant'], gthres[x$FCS.file,'cd103.gate'])
    ...
    save ( flowType.res, file =  paste("Results_SPLEEN/FlowType/", store.allFCS[i,1],"/FT_", store.allFCS[i,2],".Rdata",sep="") )
    
  }
  
  data.frame(all.props.updated, all.events.updated, t(MFI.mean.OriginalData), t(MFI.median.OriginalData), 
            t(MFI.mean.TransformedData), t(MFI.median.TransformedData))
  
}, .parallel = TRUE) # end ddply

failed.files <- which(is.na(props.events.MFIs[,ncol(props.events.MFIs)]))
if(length(failed.files) > 0){
  print("Files which failed gating")
  print(props.events.MFIs$FCS.file[failed.files])
}


save(props.events.MFIs, file = "Results_SPLEEN/PropsEventsMFIs0.Rdata")  
