...
#     points(cd45sTempb@flow.frame@exprs[RP.mac.ind, c(8,13)], col=3, cex=0.2)
    ...
    eosinTempb <-flowDensity(linneg.macneg.s, channels =c(9, 4), position = c(T, T), gates = c(gthres[i,13] , gthres[i,22])) 
    ...
    Ly6GnegTempb <-flowDensity(linneg.macneg.s, channels =c(9, 4), position = c(F, T), gates = c(gthres[i,13] , gthres[i,12])) 
    ...
    # Grab the Neu2 populationusing notSubFrame() function
    temp.flowD  <- notSubFrame(linneg.Tempb@flow.frame, channels=c(9, 4), position= "logical", gates = "missing",  Ly6GnegTempb@filter)
    neu2.flowD  <- notSubFrame(temp.flowD@flow.frame, channels=c(9, 4), position= "logical", gates = "missing", eosinTempb@filter)
    
    # Take the Neu2 population to be all cells that is not Ly6G- or EOSIN3
    all.events.updated[i,10] <- linneg.Tempb@cell.count - Ly6GnegTempb@cell.count - eosinTempb@cell.count
    all.props.updated[i,10] <- all.events.updated[i,10]/linneg.Tempb@cell.count*100
...
#     plotDens(linneg.macneg, channels = c(9,4))
#     points(linneg.Tempb@flow.frame@exprs[temp.flowD@index, c(9,4)], col = 2, cex = 0.2)
#     points(linneg.Tempb@flow.frame@exprs[neu2.flowD@index, c(9,4)], col = 3, cex = 0.2)
...
#     cd11b.plus.cd86.lo@exprs <- cd11b.plus.cd86.lo@exprs[-cd8a.typedc.Tempb@index, ]
    ...
    neu2.MFI <- sapply(1:(ncol(f.beforeTransform@exprs)-1), function(x){mean(f.beforeTransform@exprs[neu2.flowD@index, x], na.rm = T)})
    ...
    neu2.MFI <- sapply(1:(ncol(f.beforeTransform@exprs)-1), function(x){median(f.beforeTransform@exprs[neu2.flowD@index, x], na.rm = T)})
    ...
    neu2.MFI <- sapply(1:(ncol(f@exprs)-1), function(x){mean(f@exprs[neu2.flowD@index, x], na.rm = T)})
    ...
    neu2.MFI <- sapply(1:(ncol(f@exprs)-1), function(x){median(f@exprs[neu2.flowD@index, x], na.rm = T)})
