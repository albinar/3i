...
suppressWarnings ( dir.create ( "Results/After_Clean_Copy") )
...
invisible(sapply(1:length(uniqueGT), function(x){
  ...
  suppressWarnings ( dir.create ( paste("Results/After_Clean_Copy/", uniqueGT[x], sep="") ) )
...
for (i in 1:length(allFCS)){
        ...
        #set.seed(5678)
          ...
          if(class(f.clean)=="try-error")
          {
            ...
            save ( f , file =  paste("Results/After_Clean_Copy/", store.allFCS[i,1],"/AftClean_", store.allFCS[i,2],".Rdata",sep="") )
