...
datapath <- '/code/Projects/3i/Panel_BM-cell/FCS_Groups'

library(flowCore)
temp.fcs <- read.FCS("/code/Projects/3i/Panel_BM-cell/FCS_Groups/1700007K13Rik(b)_1700007K13Rik(b)/BM_L000050126_A12_003.labelled.fcs")
...
temp.fcs <- read.FCS("/code/Projects/3i/Panel_BM-cell/FCS_Groups/+_+/BM 01092015_L000096991_011.labelled.fcs")
marker.names <- temp.fcs@parameters@data$name
prop.markers=c(7:18)
...
outlierWT.Index <- which(td[,1] < -25)
## Make sure after finding the outlier.Index look into the td matrix back to look into the dimension values.
